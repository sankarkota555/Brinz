...
public class BillPdfUtil {
	...
	private static void createPdf(String text) {
		...
		try {
			...
			BarcodeQRCode barcodeQRCode = new BarcodeQRCode(BrinzConstantNames.shopName, 120, 120, null);
	...
	private static PdfPTable getHeaderTable() throws DocumentException {
		...
		try {
			...
			addressCell.setPhrase(new Phrase(BrinzConstantNames.shopAddress,BrinzFonts.footerFont));
	...
	private static PdfPTable getFooter(int spaceTop) {
		...
		PdfPCell cell = new PdfPCell(new Phrase(new Date().toString(), BrinzFonts.footerFont));
		cell.setHorizontalAlignment(Element.ALIGN_LEFT);
		cell.setBorder(Rectangle.NO_BORDER);
		footerTable.addCell(cell);

		PdfPCell cellTwo = new PdfPCell(new Phrase("Thank You", BrinzFonts.footerFont));
